...
public final class DefaultLoggingRuleConfigurationBuilder implements DefaultGlobalRuleConfigurationBuilder<LoggingRuleConfiguration, LoggingRuleBuilder> {
    ...
    public LoggingRuleConfiguration build() {
        ...
        Optional<ShardingSphereLogBuilder> sphereLogBuilder = TypedSPILoader.findService(ShardingSphereLogBuilder.class,
                                                                                         loggerFactory.getClass());

        Optional<LoggingRuleConfiguration> loggingRuleConfiguration = sphereLogBuilder.map(optional -> new LoggingRuleConfiguration(
                optional.getDefaultLoggers(loggerFactory),
                optional.getDefaultAppenders(loggerFactory)));
        return loggingRuleConfiguration.orElse(new LoggingRuleConfiguration(Collections.emptyList(),
